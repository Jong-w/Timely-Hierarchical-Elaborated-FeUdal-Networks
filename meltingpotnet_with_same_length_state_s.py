...
class MPnets(nn.Module):
    def __init__(self,
        ...
        self.hidden_s = init_hidden(args.num_workers, self.r_s * self.d,
                                    device=device, grad=True)
        self.hidden_w = init_hidden(args.num_workers, self.d * n_actions,
                                    device=device, grad=True)
    ...
    def init_obj(self):
        ...
        template_s = torch.zeros(self.b, self.d)
...
class Manager(nn.Module):
    ...
    def goal_goal_cosine(self, goals_m, states_s,  masks): #TODO changed goal_s to states_s (same with main code)
        ...
        cosine_dist = d_cos(states_s[t_s + self.c_s] - states_s[t_s], goals_m[t_m])
...
class Supervisor(nn.Module):
    def __init__(self, c_m, c_s, d, n, r_s, args, device, ):
        ...
        self.Sspace = nn.Linear(self.d, self.d)
        self.phi = nn.Linear(self.d, self.d, bias=False)
        self.Srnn = DilatedLSTM(self.d, self.d, self.r)
        self.critic = nn.Linear(self.d, 1)
...
class Worker(nn.Module):
    def __init__(self, b, c_m, c_s, d, n, k, num_actions, device):
        ...
        self.Wrnn = nn.LSTMCell(d, self.d * self.num_actions)
        self.phi = nn.Linear(self.d, self.d, bias=False)

        self.critic = nn.Sequential(
            nn.Linear(d * num_actions, 50),
            nn.ReLU(),
            nn.Linear(50, 1)
        )
    ...
    def forward(self, z, goals_s, hidden, mask):
        ...
        u = u.reshape(u.shape[0], self.d, self.num_actions)
...
def mp_loss(storage, next_v_m, next_v_s, next_v_w, args):
    ...
    loss_supervisor = (state_goal_cosines * advantage_m.detach()).mean()
    loss_manager = (goal_goal_cosines * advantage_s.detach()).mean()
