...
def experiment(args):
    ...
    save_models = list(torch.arange(0, int(args.max_steps),
                                   int(args.max_steps) // 20).numpy())
            ...
            for _i in range(len(done)):
                if done[_i]:
                    ...
                    {"training/episode/reward": info[_i]['returns/episodic_reward'],
                     ...
                     "training/episode/reward_sign": int(info[_i]['returns/episodic_reward']!=-100)
                     },step=step)
        ...
        if len(save_steps) > 0 and step > save_steps[0]:
            ...
            while True:
                ...
                if info['returns/episodic_reward'] is not None:
                    ...
                    step_t_ep += 1
                    wandb.log(
                        {"test/episode/reward": reward, "test/episode/length": length, "test/episode/reward_sign": int(reward!=-100)},step=step)
                    step_trueepi += 1
